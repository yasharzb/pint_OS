...
- `priority_lock` این قفل برای تغییر thread-safe مقدار `effective_priority` استفاده می‌شود.
...
*در هر دو تابع `lock_acquire` و `lock_release` برای جلوگیری از race condition در ابتدا با استفاده از `intr_disable` اینتراپت‌ها را غیرفعال و در انتهای آن با استفاده از `intr_set_level` اینتراپت‌ها را به حالت قبل باز می‌گردانیم.
...
چون تابع `calculate_priority_and_yield` و `compare_priority_and_update` می‌توانند از ریسه‌های مختلف صدا زده شوند و مقدار `effective_priority` را تغییر دهند ممکن است race condition رخ دهد. به همین علت در این توابع و هنگام خواندن اولویت موثر قبل از دسترسی به این متغیر قفل `priority_lock` مربوط به ریسه گرفته می‌شود و پس از آن آزاد می‌شود.

### منطق

>> پرسش هفتم: چرا این طراحی را استفاده کردید؟ برتری طراحی فعلی خود را بر طراحی‌های دیگری که مدنظر داشته‌اید بیان کنید.

طراحی دیگری که مدنظر داشتیم به جای آپدیت اولویت موثر هنگام تاخیر، هنگام نیاز به آن، به طور بازگشتی آن را محاسبه می‌کرد. مشکل این طراحی هزینه‌ی بالای محاسبه هنگام به دست آوردن ریسه با بالاترین اولویت (برای scheduling یا بیدار کردن ریسه‌ی منتظر قفل و ...) بود. در طراحی فعلی این هزینه کم‌تر شده است.
...
در تابع `switch_threads` چهار ثبات الزامی برای اجرای ریسه در پشته‌ی همان ریسه ذخیره می‌شود (ثبات‌های `esi`, `edi`, `ebp`, `ebx`). این تابع دو ورودی `next`,`cur` را می‌گیرد که ریسه‌هایی است که قرار است بین آن‌ها تعویض قرار بگیرد.  خود `esp` برای ریسه `cur` در متغیر `stack` مربوط به شی آن ریسه (`object`) ذخیره می‌شود یا به عبارت ساده‌تر در `cur->stack`. مقدار `esp` برای ریسه جدید در `next->stack` قرار دارد که در این تابع این مقدار در نهایت در ثبات پردازنده می‌نشیند. پس از این عمل، ۴ ثبات الزامی ریسه `next` که در دفعه قبلی که از ریسه `next` به ریسه دیگری تعویض کردیم در پشته `next` قرارداده شده است بازیابی می‌کنیم. بدین ترتیب ریسه `next` به همان حالتی برمی‌گردد که زمانی در حال اجرا بود.

هنگامی که تابع `switch_threads` شروع به اجرا می‌کند `program counter` به صورت خودکار در خانه‌ی اول استک ترد فعلی ذخیره می‌شود و پس از اینکه کار تابع تمام شود با دستور `ret` به صورت خودکار با پاپ کردن ۴ بایت از استک ترد فعلی، از تابع به دستور بعد از فراخوانی این تابع برمی‌گردد.
...
این تابع در تابع `timer_interrupt` در پرونده‌ی `time.c` استفاده می‌شود. تابع `thread_tick` یک پرچم برای `interrupt handler` مشخص می‌کند که پیش از خروج از فضای رسیدگی به `interrupt` به ترد جدید منتقل شوند. به  `interrupt` ها در تابع `intr_handler` از پرونده‌ی `interrupt.c` رسیدگی می‌شود. این تابع در نهایت تابع `intr_entry` را از پرونده‌ی `intr-stubs.S` صدا می‌زند. در این قسمت از تابع مذکور وارد محیط کرنل می‌شویم:
...
ولی همانطور که در سوال ۸ گفته شد که در تابع `switch_threads` عملا اتفاق زیر می‌افتد:
...
پس هنگاهی که یک ترد در حال اجرا است در پشته‌ی مربوط به آن ترد هستیم. پس این تابع هم در پشته‌ی تردی که این `interrupt` را فراخوانده است اجرا می‌شود.
...
تست sema-up-priority-donate.c به تست ها با همین سناریو اضافه شد.
