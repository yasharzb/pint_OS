...
هنگامی که تابع `switch_threads` صدا زده می‌شود `program counter` که همان return address است به صورت خودکار در استک ترد فعلی ذخیره می‌شود و پس از اینکه کار تابع تمام شود با دستور `ret` به صورت خودکار با پاپ کردن ۴ بایت از استک ترد، از تابع به دستور بعدی تردی که می‌خواهد اجرا شود می‌رود.
...
این تابع در تابع `timer_interrupt` در پرونده‌ی `time.c` استفاده می‌شود. تابع `thread_tick` یک پرچم برای `interrupt handler` مشخص می‌کند که پیش از خروج از فضای رسیدگی به `interrupt` به ترد جدید منتقل شوند. به  `interrupt`
 ها در تابع `intr_handler` از پرونده‌ی `interrupt.c` رسیدگی می‌شود. این تابع توسط تابع `intr_entry` را از پرونده‌ی `intr-stubs.S` صدا زده می‌شود. 
 
 این تابع در پشته‌ی کرنل اجرا می‌شود. تغییر پشته توسط CPU انجام می‌شود و علت آن این است که پشته‌ی کاربر ممکن است صحیح نباشد.

 در این قسمت از تابع مذکور وارد محیط کرنل می‌شویم:
