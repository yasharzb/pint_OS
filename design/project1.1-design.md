...
این اعضا در پیاده‌سازی سیستم کال‌ `wait` و آزاد کردن منابع ترد کاربرد دارند:
...
با استفاده از سمافور `exited` در استراکت ترد فرزند این کار را انجام داده‌ایم. پردازه‌ی فرزند پس از قرار دادن `exit_value` ی خود در استراکت ترد خود سمافور `exited` را up می‌کند. 
...
با چک کردن آرگومان‌های سیستم‌کال‌ها نیز از مشکل‌هایی که ممکن است به خاطر این مسئله پیش بیاید (مثلا استرینگی که با صفر ترمینیت نشده)
جلوگیری می‌شود.
