...
با توجه به اینکه حافظه‌ی نهان هنگام دسترسی از روی لیست `cached_blks` گرفته می‌شود می‌توان از قفل تعریف شده در ساختار `cache` بهره برد.
...
من متوجه این سوال نشدم. آیا ایرادی دارد که در انتهای تابع `inode_read_at` تابع `inode_read_ahead` صدا زده شود؟
