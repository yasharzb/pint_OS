...
tid_t process_execute(const char *cmdline)
{
    ...
    /* get filename from cmdline for setting thread name */

    // TODO Maybe we can refactor this so that we only tokenize once here
    // and pass it to start process and load
...
bool load(const char *cmdline, void (**eip)(void), void **esp)
{
    ...
    strlcpy(commands, cmdline, PGSIZE);
    char *saved_pointer = NULL;
    argv = palloc_get_page(0);
    if (argv == NULL)
        goto done;

    file_name = strtok_r(commands, " ", &saved_pointer);
