...
void inode_read_ahead(void *sec_idx_ptr)
{
  block_sector_t index = *(int *)sec_idx_ptr;
  cache *ca = get_cache(index + 1);
  uint8_t buffer[BLOCK_SECTOR_SIZE];

  if (ca == NULL)
  {
    block_read(fs_device, index + 1, buffer);
    set_cache(index + 1, buffer);
  }
