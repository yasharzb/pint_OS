...
inode_open(block_sector_t sector)
{
  ...
  block_read(fs_device, inode->sector, &inode->data, false);
  return inode;
}
...
bool extend_inode_disk_to_size(block_sector_t inode_disk_sector, struct inode_disk *disk_inode, off_t size)
{
  ...
  block_read(fs_device, disk_inode->double_indirect_block, double_indirect_block_buffer, false);
  ...
  block_write(fs_device, disk_inode->double_indirect_block, double_indirect_block_buffer, false);
  ...
  for (off_t sector_to_build = current_sectors; sector_to_build < sectors_needed; sector_to_build++)
  {
    ...
    if (last_indirect_block_ind != indirect_block_ind)
    {
      if (last_indirect_block_ind != -1)
        block_write(fs_device, double_indirect_block_buffer->blocks[last_indirect_block_ind], indirect_block_buffer, false);
      last_indirect_block_ind = indirect_block_ind;
      block_read(fs_device, double_indirect_block_buffer->blocks[indirect_block_ind], indirect_block_buffer, false);
    }
  ...
  if (success)
  {
    ...
    block_write(fs_device, inode_disk_sector, disk_inode, false);
  }
  return success;
}
...
void inode_close(struct inode *inode)
{
  ...
  if (--inode->open_cnt == 0)
  {
    ...
    if (inode->removed)
    {
      ...
      block_read(fs_device, inode->data.double_indirect_block, double_indirect_block_buffer, false);
...
off_t inode_read_at(struct inode *inode, void *buffer_, off_t size, off_t offset)
{
  ...
  while (size > 0)
  {
    ...
    sector = indirect_block_buffer->blocks[sector_ind_in_indirect_block];
...
off_t inode_read_at_ahead(struct inode *inode, void *buffer_, off_t size, off_t offset)
{
  uint8_t *buffer = buffer_;
  off_t bytes_read = 0;
